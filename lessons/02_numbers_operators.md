...
```python
result = 5 >= 3                               # значение переменной будет True
another_result = 3 >= 5                       # значение переменной будет False
compare_the_same_values = 3 >= 3              # значение переменной будет True
small_value = 4
big_value = 6
compare_variables = big_value >= small_value  # значение переменной будет True
compare_variables = small_value >= big_value  # значение переменной будет False
compare_strings = 'ab' >= 'baa'               # значение переменной будет False
```

`<=` - меньше или равно

```python
result = 5 <= 3                                             # значение переменной будетTrue
another_result = 3 <= 5                                     # значение переменной будет False
compare_the_same_values = 3 <= 3                            # значение переменной будет True
small_value = 4
big_value = 6
compare_variables = big_value <= small_value                # значение переменной будет False
compare_variables = small_value <= big_value                # значение переменной будет True
compare_strings = 'ab' <= 'baa'                             # значение переменной будет True
```

`==` - равно (по значению)

```python
result = 5 == 3                                             # значение переменной будет False
another_result = 3 == 5                                     # значение переменной будет False
compare_the_same_values = 3 == 3                            # значение переменной будет True
small_value = 4
big_value = 6
...
Если одно значение в выражении при приведении дает `True`, то и результат будет `True`. Возвращает первое значение,
которое можно быть приведено к `True`. Если все значения приводятся к `False`, то и результат будет `False`.
...
В Python для обозначения блоков кода мы используем обычные отступы. Во многих языках программирования используют
для этого различные скобки. У нас используется 4 пробела (на большинстве компьютеров кнопка tab работает именно 
как 4 пробела)

### if

Рассмотрим ключевые слова **if**, **elif**, **else**, после них блок кода выделяется 4 пробелами.
...
Если нам нужно разделить выполнение кода на то, что делать при выполнении условии и при невыполнении условия, можно
...
1. Пользователь задает переменную возраст. Если он старше 18 лет, выведите сообщение, что все хорошо, если нет, то 
выведите сообщение, что не все хорошо.
2. Добавить к первому условию, если возраст больше чем 100, распечатать текст, что пользователь вводит нас в заблуждение
3. Добавить вывод, является ли введенный возраст четным или нечетным.
4. Добавить, что пользователь вводит еще и имя. И если в имени есть буква 'a', то написать, что мы даже не собираемся его
   проверять.
5. Проверить если в имени есть буква 'v' или большая, или маленькая не важно. И возраст пользователя четный, то написать, что он
   выиграл приз, если нет, то не выиграл.
6. Спросить пользователя возраст, пол и имя. Для всех младше 15 мы пишем, что рекомендуем теннис, для парней старше 15
